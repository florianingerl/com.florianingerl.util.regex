...
final public class POSIX_Unicode {
	...
	public static boolean isBlank(int ch) {
		...
		return isSpace(ch) && ch != 0xa & ch != 0xb && ch != 0xc && ch != 0xd && ch != 0x85
	...
	public static boolean isGraph(int ch) {
		int type = Character.getType(ch);
		return !(isSpace(ch) || Character.CONTROL == type || Character.SURROGATE == type
	...
	public static boolean isWord(int ch) {
		return isAlpha(ch) || ((((1 << Character.NON_SPACING_MARK) | (1 << Character.ENCLOSING_MARK)
				| (1 << Character.COMBINING_SPACING_MARK) | (1 << Character.CONNECTOR_PUNCTUATION)) >> Character
						.getType(ch))
				& 1) != 0 || isDigit(ch) || isJoinControl(ch);
	}
}
