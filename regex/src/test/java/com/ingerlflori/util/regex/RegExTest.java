...
public class RegExTest {
	 ...
	 * supplementary characters. This method does NOT fully take care of the
	 * regex syntax.
	 */
	...
	private static void twoFindIndexes(String input, Matcher matcher, int a, int b) throws Exception {
		matcher.reset(input);
		matcher.find();
		if (matcher.start() != a)
			failCount++;
	...
	private static void nonCaptureRepetitionTest() throws Exception {
		...
		String[] patterns = new String[] { "(?:\\w{4})+;", "(?:\\w{8})*;", "(?:\\w{2}){2,4};", "(?:\\w{4}){2,};", // only
																													// matches
																													// the
	...
	private static void javaCharClassTest() throws Exception {
		for (int i = 0; i < 1000; i++) {
			...
			check("{javaJavaIdentifierStart}", c, Character.isJavaIdentifierStart(c));
			check("{javaJavaIdentifierPart}", c, Character.isJavaIdentifierPart(c));
			check("{javaUnicodeIdentifierStart}", c, Character.isUnicodeIdentifierStart(c));
			check("{javaUnicodeIdentifierPart}", c, Character.isUnicodeIdentifierPart(c));
			check("{javaIdentifierIgnorable}", c, Character.isIdentifierIgnorable(c));
		...
		// Supplementary character test
		for (int i = 0; i < 1000; i++) {
			int c = generator.nextInt(Character.MAX_CODE_POINT - Character.MIN_SUPPLEMENTARY_CODE_POINT)
	...
	/*
	 * private static void numOccurrencesTest() throws Exception { Pattern
	 * pattern = Pattern.compile("aaa");
	 * 
	 * if (pattern.numOccurrences("aaaaaa", false) != 2) failCount++; if
	 * (pattern.numOccurrences("aaaaaa", true) != 4) failCount++;
	 * 
	 * pattern = Pattern.compile("^"); if (pattern.numOccurrences("aaaaaa",
	 * false) != 1) failCount++; if (pattern.numOccurrences("aaaaaa", true) !=
	 * 1) failCount++;
	 * 
	...
	private static void replaceFirstTest() {
		...
		if (!matcher.replaceFirst(toSupplementaries("test")).equals(toSupplementaries("testzzzabcczzzabccc")))
			failCount++;

		matcher.reset(toSupplementaries("zzzabccczzzabcczzzabccczzz"));
		if (!matcher.replaceFirst(toSupplementaries("test")).equals(toSupplementaries("zzztestzzzabcczzzabccczzz")))
			failCount++;
	...
	private static void unixLinesTest() {
		...
		pattern = Pattern.compile(toSupplementaries("[az]$"), Pattern.MULTILINE | Pattern.UNIX_LINES);
	...
	private static void caseFoldingTest() { // bug 4504687
		...
		String[] texts = new String[] { "A", "\u00c0", "\u0410", "AB", "\u00c0\u00c1", "\u0410\u0411", "A", "\u00c0",
				"\u0410", "B", "\u00c2", "\u0411", "aA", "\u00e0\u00c0", "\u0430\u0410" };

		boolean[] expected = new boolean[] { true, false, false, true, false, false, true, false, false, true, false,
	...
	private static void splitTest() {
		...
		String[][] input = new String[][] { { " ", "Abc Efg Hij" }, // normal
																	// non-zero-match
				{ " ", " Abc Efg Hij" }, // leading empty str for non-zero-match
				{ " ", "Abc  Efg Hij" }, // non-zero-match in the middle
				{ "(?=\\p{Lu})", "AbcEfgHij" }, // no leading empty str for
												// zero-match
				{ "(?=\\p{Lu})", "AbcEfg" }, { "(?=\\p{Lu})", "Abc" }, { " ", "" }, // zero-length
																					// input
				{ ".*", "" },
				...
				{ "1", "awgqwefg1fefw4vssv1vvv1" }, { "1", "a\u4ebafg1fefw\u4eba4\u9f9cvssv\u9f9c1v\u672c\u672cvv" },
				{ "\u56da", "1\u56da23\u56da456\u56da7890" },
				{ "\u56da", "1\u56da23\u9f9c\u672c\u672c\u56da456\u56da\u9f9c\u672c7890" }, { "\u56da", "" },
				{ "[ \t,:.]", "This is,testing: with\tdifferent separators." }, // multiple
																				// septs
				{ "o", "boo:and:foo" }, { "o", "booooo:and:fooooo" }, { "o", "fooooo:" }, };

		String[][] expected = new String[][] { { "Abc", "Efg", "Hij" }, { "", "Abc", "Efg", "Hij" },
				{ "Abc", "", "Efg", "Hij" }, { "Abc", "Efg", "Hij" }, { "Abc", "Efg" }, { "Abc" }, { "" }, { "" },

				{ "awgqwefg1fefw", "vssv1vvv1" }, { "afbfq", "bgwgb", "wngnwggw", "", "hjrnhneerh" },
				{ "awgqwefg", "fefw4vssv", "vvv" }, { "a\u4ebafg", "fefw\u4eba4\u9f9cvssv\u9f9c", "v\u672c\u672cvv" },
				{ "1", "23", "456", "7890" }, { "1", "23\u9f9c\u672c\u672c", "456", "\u9f9c\u672c7890" }, { "" },
				{ "This", "is", "testing", "", "with", "different", "separators" }, { "b", "", ":and:f" },
				{ "b", "", "", "", "", ":and:f" }, { "f", "", "", "", "", ":" }, };
		for (int i = 0; i < input.length; i++) {
			...
			if (input[i][1].length() > 0 && // splitAsStream() return empty
											// resulting
											// array for zero-length input for
											// now
					!Arrays.equals(pattern.splitAsStream(input[i][1]).toArray(), expected[i])) {
				failCount++;
			}
		}
		report("Split");
	...
	private static void longPatternTest() throws Exception {
		try {
			Pattern pattern = Pattern.compile("a 32-character-long pattern xxxx");
		...
		// Supplementary character test
		try {
			Pattern pattern = Pattern.compile(toSupplementaries("a 32-character-long pattern xxxx"));
			...
			for (int i = 0; i < 100; i++)
				patternToBe.append(Character.toChars(Character.MIN_SUPPLEMENTARY_CODE_POINT + 97 + i % 26));
			pattern = Pattern.compile(patternToBe.toString());
		} catch (PatternSyntaxException e) {
			failCount++;
		}
	...
	private static void charClassTest() throws Exception {
		...
		try {
			...
			pattern = Pattern.compile("[ab\u00ffcd]", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
			check(pattern, "ab\u00ffcd", true);
			check(pattern, "Ab\u0178Cd", true);

			// u00b5 when UNICODE_CASE
			pattern = Pattern.compile("[ab\u00b5cd]", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
		...
		/*
		 * Special cases (1)LatinSmallLetterLongS u+017f
		 * (2)LatinSmallLetterDotlessI u+0131
		 * (3)LatineCapitalLetterIWithDotAbove u+0130 (4)KelvinSign u+212a
	...
	/**
	 * Unicode Technical Report #18, section 2.6 End of Line There is no empty
	 * line to be matched in the sequence \u000D\u000A but there is an empty
	...
	private static void globalSubstitute() throws Exception {
		...
		p = Pattern.compile(toSupplementaries("(ab)(c*)"));
		m = p.matcher(toSupplementaries("abccczzzabcczzzabccc"));
		if (!m.replaceAll(toSupplementaries("test")).equals(toSupplementaries("testzzztestzzztest")))
			failCount++;

		m.reset(toSupplementaries("zzzabccczzzabcczzzabccczzz"));
		if (!m.replaceAll(toSupplementaries("test")).equals(toSupplementaries("zzztestzzztestzzztestzzz")))
			failCount++;
	...
	/**
	 * Tests the usage of Matcher.appendReplacement() with literal and group
	 * substitutions.
	 */
	...
	/*
	 * 5 groups of characters are created to make a substitution string. A base
	 * string will be created including random lead chars, the substitution
	 * string, and random trailing chars. A pattern containing the 5 groups is
	 * searched for and replaced with: random group + random string + random
	 * group. The results are checked for correctness.
	 */
	...
	/**
	 * Checks the handling of some escape sequences that the Pattern class
	 * should process instead of the java compiler. These are not in the file
	 * because the escapes should be be processed by the Pattern class when the
	 * regex is compiled.
	 */
	...
	/**
	 * Checks the handling of blank input situations. These tests are
	 * incompatible with my test file format.
	 */
	...
	/**
	 * Tests the Boyer-Moore pattern matching of a character sequence on
	...
	/**
	 * Tests the matching of slices on randomly generated patterns. The
	 * Boyer-Moore optimization is not done on these patterns because it uses
	...
	/**
	 * Goes through the file "TestCases.txt" and creates many patterns described
	 * in the file, matching the patterns against input lines in the file, and
	 * comparing the results against the correct results also found in the file.
	 * The file format is described in comments at the head of the file.
	 */
	...
	/**
	 * Reads a line from the input file. Keeps reading lines until a non empty
	 * non comment line is read. If the line contains a \n then these two
	 * characters are replaced by a newline char. If a \\uxxxx sequence is read
	 * then the sequence is replaced by the unicode char.
	 */
	...
	private static void check(Pattern p, String s, String g, String expected) {
		...
		m.find();
		if (!m.group(g).equals(expected) || s.charAt(m.start(g)) != expected.charAt(0)
	...
	private static void namedGroupCaptureTest() throws Exception {
		...
		check(Pattern.compile(toSupplementaries("(a)(b)(c)(d)(e)(f)(g)(h)(i)(j)") + "(?<gname>"
				+ toSupplementaries("k)") + "\\k<gname>"), toSupplementaries("abcdefghijkk"), true);
		...
		checkReplaceFirst("(?<gn1>" + toSupplementaries("ab") + ")(?<gn2>" + toSupplementaries("c") + "*)",
				toSupplementaries("abccczzzabcczzzabccc"), "${gn1}", toSupplementaries("abzzzabcczzzabccc"));

		checkReplaceAll("(?<gn1>" + toSupplementaries("ab") + ")(?<gn2>" + toSupplementaries("c") + "*)",
				toSupplementaries("abccczzzabcczzzabccc"), "${gn1}", toSupplementaries("abzzzabzzzab"));

		checkReplaceFirst("(?<gn1>" + toSupplementaries("ab") + ")(?<gn2>" + toSupplementaries("c") + "*)",
				toSupplementaries("abccczzzabcczzzabccc"), "${gn2}", toSupplementaries("ccczzzabcczzzabccc"));

		checkReplaceAll("(?<gn1>" + toSupplementaries("ab") + ")(?<gn2>" + toSupplementaries("c") + "*)",
				toSupplementaries("abccczzzabcczzzabccc"), "${gn2}", toSupplementaries("ccczzzcczzzccc"));
	...
	private static void unicodePropertiesTest() throws Exception {
		// different forms
		if (!Pattern.compile("\\p{IsLu}").matcher("A").matches() || !Pattern.compile("\\p{Lu}").matcher("A").matches()
	...
	private static void unicodeHexNotationTest() throws Exception {
		...
		for (int cp = 0; cp <= 0x10FFFF; cp++) {
			...
			String hexUTF16 = (cp <= 0xFFFF) ? String.format("\\u%04x", cp)
					: String.format("\\u%04x\\u%04x", (int) Character.toChars(cp)[0], (int) Character.toChars(cp)[1]);
	...
	private static void unicodeClassesTest() throws Exception {
		...
		for (int cp = 1; cp < 0x30000; cp++) {
			...
			POSIX_ASCII.isLower(cp) != lower.reset(str).matches()
					...
					// blank
					POSIX_ASCII.isType(cp, POSIX_ASCII.BLANK) != blank.reset(str).matches()
	...
	private static void horizontalAndVerticalWSTest() throws Exception {
		String hws = new String(new char[] { 0x09, 0x20, 0xa0, 0x1680, 0x180e, 0x2000, 0x2001, 0x2002, 0x2003, 0x2004,
		...
		if (!Pattern.compile("\\h+").matcher(hws).matches() || !Pattern.compile("[\\h]+").matcher(hws).matches())
			failCount++;
		if (Pattern.compile("\\H").matcher(hws).find() || Pattern.compile("[\\H]").matcher(hws).find())
			failCount++;
		if (!Pattern.compile("\\v+").matcher(vws).matches() || !Pattern.compile("[\\v]+").matcher(vws).matches())
			failCount++;
		if (Pattern.compile("\\V").matcher(vws).find() || Pattern.compile("[\\V]").matcher(vws).find())
			failCount++;
		String prefix = "abcd";
		String suffix = "efgh";
		String ng = "A";
	...
	private static void linebreakTest() throws Exception {
		...
		String crnl = "\r\n";
		if (!Pattern.compile("\\R+").matcher(linebreaks).matches() || !Pattern.compile("\\R").matcher(crnl).matches()
	...
	private static void branchTest() throws Exception {
		if (!Pattern.compile("(a)?bc|d").matcher("d").find() || // greedy
				!Pattern.compile("(a)+bc|d").matcher("d").find() || !Pattern.compile("(a)*bc|d").matcher("d").find()
	...
	private static void groupCurlyNotFoundSuppTest() throws Exception {
		...
		for (String pStr : new String[] { "test(.)+(@[a-zA-Z.]+)", "test(.)*(@[a-zA-Z.]+)", "test([^B])+(@[a-zA-Z.]+)",
				"test([^B])*(@[a-zA-Z.]+)", "test(\\P{IsControl})+(@[a-zA-Z.]+)",
				"test(\\P{IsControl})*(@[a-zA-Z.]+)", }) {
			Matcher m = Pattern.compile(pStr, Pattern.CASE_INSENSITIVE).matcher(input);
			try {
				if (m.find()) {
					failCount++;
				}
